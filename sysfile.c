...
argfd(int n, int *pfd, struct file **pf)
{
	...
	if(fd < 0 || fd >= NOFILE || (f=proc->ofile[fd]) == 0){
		werrstr(EINOTOPEN, strlen(EINOTOPEN));
		return -1;
	}
	if(pfd)
		*pfd = fd;
...
sys_seek(void)
{
	...
	if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argint(2, &dir) < 0)
		return -1;
	return fileseek(f, n, dir);
}
...
create(char *path, short type, short major, short minor)
{
	...
	ip->nlink = 1;
	ip->owner = proc->uid;
	ip->perms = W_READ|U_READ|U_WRITE; 
	iupdate(ip);
...
int // return short; fname, mode, perms
sys_chperms(void)
{
	int rmode;
	int perms;
